...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private BusinessDomainDslLexer lexer;
	private BusinessDomainDslParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public BusinessDomainDslLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new BusinessDomainDslLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public BusinessDomainDslParser getParser() {
		if (parser == null) {
			parser = new BusinessDomainDslParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "org::mod4j::businessdomain::xtext::Linking");
