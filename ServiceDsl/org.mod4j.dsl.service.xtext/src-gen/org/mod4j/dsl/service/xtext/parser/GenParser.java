...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private ServiceDslLexer lexer;
	private ServiceDslParser parser;
	...
	public ServiceDslLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new ServiceDslLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public ServiceDslParser getParser() {
		if (parser == null) {
			parser = new ServiceDslParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "org::mod4j::dsl::service::xtext::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"org::mod4j::dsl::service::xtext::GenChecks","org::mod4j::dsl::service::xtext::Checks"};
