...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	public DataContractDslLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new DataContractDslLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public DataContractDslParser getParser() {
		if (parser == null) {
			parser = new DataContractDslParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "org::mod4j::dsl::datacontract::xtext::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"org::mod4j::dsl::datacontract::xtext::GenChecks","org::mod4j::dsl::datacontract::xtext::Checks"};
