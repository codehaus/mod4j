 ...
 *     Ordina - made changes to the original WorkflowRunner to allow acces to the Issues from Java code
 *******************************************************************************/
...
//				runner.logger.error("can't find the workflow file '" + line.getArgs()[0] + "' in the current class path");
		...
		public boolean prepare(final String workFlowFile, final ProgressMonitor theMonitor, final Map<String, String> theParams) {
			...
			logger.info("--------------------------------------------------------------------------------------");
			logger.info("openArchitectureWare " + getVersion());
			logger.info("(c) 2005-2008 openarchitectureware.org and contributors");
			logger.info("--------------------------------------------------------------------------------------");
