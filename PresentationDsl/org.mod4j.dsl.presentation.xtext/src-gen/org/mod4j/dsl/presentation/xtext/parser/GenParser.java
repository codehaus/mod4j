...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private PresentationDslLexer lexer;
	private PresentationDslParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public PresentationDslLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new PresentationDslLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public PresentationDslParser getParser() {
		if (parser == null) {
			parser = new PresentationDslParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "org::mod4j::dsl::presentation::xtext::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"org::mod4j::dsl::presentation::xtext::GenChecks","org::mod4j::dsl::presentation::xtext::Checks"};
